...
from .prompts import output_file_check
...
function_schema = {
  "name": "run_code",
  "description":
  "Executes code on the user's machine and returns the output, if any plots are generated, only return the filenames, do not show the plots.",
  "parameters": {
    "type": "object",
    "properties": {
      "language": {
...
class Interpreter:
  ...
  def respond(self):
    ...
    self.think_step += 1
    if not self.additional_system_message or self.data_path != os.environ.get('data', '.data/finance.csv'):
      import pandas as pd
      df = pd.read_csv(os.environ.get('data', '.data/finance.csv'))
      description = df.describe().to_string()
      first_rows = df.head(5).to_string()
      self.additional_system_message = "The data is located locally in current directory at " + os.environ.get('data', '.data/finance.csv') + " . use plotly if you need to plot graph and output using plotly.io.write_json to .output/ folder in json format" + \
        "After you are done, say Here is your asnwer and show the result. Don't tell the user where you stored the output data" + \
        ...
        "When you calculate Revenue or Profit, remember to convert revenue sum to positive. Apply your financial knowledge to best translate user intent into program. The data description is " + description + " and the first 5 rows are " + first_rows
  
    messages = tt.trim(self.messages, self.model, system_message=self.system_message + self.additional_system_message)
    ...
    for chunk in response:
      ...
      if 'function_call' in self.messages[-1]:
        ...
        in_function_call = False
        if "Here is" not in self.messages[-1]['content']:
          process_box.markdown(self.messages[-1]['content'])
        else:
          result_box.markdown(self.messages[-1]['content'])
      ...
      if chunk["choices"][0]["finish_reason"]:
        if chunk["choices"][0]["finish_reason"] == "function_call":
          if self.debug_mode:
            ...
            process_box.code(self.messages[-1]['function_call']['parsed_arguments']['code'], language=self.messages[-1]['function_call']['parsed_arguments']['language'])
          ...
          self.last_ran_code = self.messages[-1]["function_call"]["parsed_arguments"]["code"]
