...
def monthly_revenue_trend(df):
    ...
    for prefix in grouped_revenue.columns:
        plt.plot(
            grouped_revenue.index, grouped_revenue[prefix], marker="o", label=prefix
        )
...
def monthly_expense_breakdown(df):
    ...
    plt.figure(figsize=(14, 8))
    grouped_expenses.transpose().plot(
        kind="bar", stacked=True, figsize=(14, 8), colormap="tab20", ax=plt.gca()
    )
...
def monthly_net_profit_loss(df):
    ...
    profit = df[df["Item"] == "Revenue"][month_columns].sum(
        numeric_only=True
    ).abs() - df[df["Item"] != "Revenue"][month_columns].sum(numeric_only=True)
